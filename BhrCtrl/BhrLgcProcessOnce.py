...
def processOneInputGiveOneInstruction():
    ...
    is_findingToTalk = BhrLgcManualProcess.parse_isFindingPeopletoTalk(input_from_java[3]) # Last action is finding people to talk
    is_idling = BhrLgcManualProcess.parse_isIdling(input_from_java[3])
    is_talking = BhrLgcManualProcess.parse_isTalking(input_from_java[3]) 
    is_buying = BhrLgcManualProcess.parse_isBuying(input_from_java[3])
    is_talk_instruction = False

    if is_findingToTalk:
        print ('Last action is finding people to talk, next action should be talking')
        target_sleeping, sleep_target_name = BhrLgcManualProcess.parse_target_sleeping(input_from_java[3]) 
        if target_sleeping:
            # action: move, npcid 这个人，在surrounding 睡觉，做下个动作，npc在睡觉进memeories
            print('I am going to find someone to talk to, and that person is sleeping')
            ...
            instruction_in_human += f" I went to the {sleep_target_name} but he is sleeping, I am going to do something else now."
            is_talk_instruction = False
        else:
        ...
        shop_target_present, shopowner_target_name  = BhrLgcManualProcess.parse_target_oid_owner_at_shop(input_from_java[3])
        ...
        if BhrLgcGPTProcess.needDeepTalk(memories_str, prior_reflection_str, inputInHumanString, instruction_in_human, npcId):
            ...
            words_to_say = BhrLgcGPTProcess.generateThreeSentencesForAction(memories_str, prior_reflection_str, cur_schedule_str, instruction_in_human, npcId)
        is_talk_instruction = False
    # elif is_idling and is_talking:
    #     print ('Not idling but talking, continue current conversation')
    #     # Do nothing in this case, just ingore the input
