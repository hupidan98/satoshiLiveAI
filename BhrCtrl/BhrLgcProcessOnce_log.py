...
# A helper class to redirect stdout/stderr to logger
...
def processOneInputGiveOneInstruction():
    ...
    try:
            ...
            if instruction_in_human != '':
                ...
                while retry < 3:
                    try:
                        ...
                        logger.info(instruction_to_give)
                        ...
                        logger.info("Error snstruction to parse: ")
                        retry += 1
                        if retry == 3:
                            # If we fail after 3 retries, raise the exception to break
                            raise
        ...
        # If an exception occurs, log it and then re-raise it to break execution
        if logger:
