...
# Define sock as a global variable
sock = None
...
# def receive_input_long(sock):
#     global sock  # Refer to the global sock so it can be updated if reconnection is needed
#     while True:
#         try:
#             length_bytes = sock.recv(4)
            
#             if not length_bytes:  # If length_bytes is empty, connection might be lost
...
def reconnect_and_replace_socket(ip_txt, port_int):
    global sock, config  # Use global variables to update the existing sock and config
    print("Reconnecting socket...")
    new_sock, new_config = reconnect_socket(ip_txt, port_int)
    
    if new_sock is None:
        print("Reconnection failed. Exiting.")
        exit(1)  # or handle as needed
    
    # Update the global variables with the new connection
    sock = new_sock
    config = new_config
