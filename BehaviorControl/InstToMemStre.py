...
def InstToMemStreDB(input_from_java, instruction):
    ...
    output_str = GPTProcess.InstructionToHumanString(instruction)
    insert_npcId = input_from_java[1]
    insert_time = input_from_java[0]
    
    insert_content = output_str
    insert_importance = GPTProcess.get_importance(output_str)
    insert_embedding= GPTProcess.get_embedding(output_str)
    insert_isInstruction = 1

    MemStreDB.insert_into_table(memstre_db_connection, insert_npcId, insert_time, insert_isInstruction,insert_content, insert_importance, insert_embedding)
    return 0
...
def InstToMemStreSatoshiDB(input_from_java, words):
    ...
    insert_importance = GPTProcess.get_importance(words)
    insert_embedding= GPTProcess.get_embedding(words)
    insert_isInstruction = 1
